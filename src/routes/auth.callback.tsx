  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session) {
        // The login persists in localStorage and propagates to other open tabs
        // automatically via the storage event — the original tab will sign in.
        markDone();
      }
    });
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-background text-foreground px-6 text-center">
        ...
        <p className="mt-4 max-w-sm text-sm leading-relaxed text-muted-foreground">
          Te hemos iniciado sesión en la pestaña donde pediste el enlace.
          Vuelve allí para seguir leyendo.
        </p>
        <button
