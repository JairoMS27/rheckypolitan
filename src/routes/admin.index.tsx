...
function Inner() {
  ...
  const remove = async (issue: Issue) => {
    if (!confirm(`¿Borrar el N.º ${issue.number}? Se eliminarán también todas sus páginas.`)) return;
    // delete storage folder
    const { data: files } = await supabase.storage.from("magazines").list(issue.id, { limit: 1000 });
          ...
          {issues.map((i) => (
            <li key={i.id} className="flex items-center gap-6 py-4">
              <div className="h-20 w-16 shrink-0 bg-muted">
                {i.cover_path && (
                  <img src={publicUrl(i.cover_path)} alt="" className="h-full w-full object-cover" />
                )}
              </div>
              ...
              <div className="flex gap-2">
                <Link to="/revista/$number" params={{ number: String(i.number) }}>
                  <Button variant="outline" size="sm">Ver</Button>
                </Link>
                <Link to="/admin/$id/edit" params={{ id: i.id }}>
