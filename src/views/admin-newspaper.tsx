  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from("posts")
        .select(
          "id,section,slug,title,excerpt,content_html,cover_path,cover_position,author,published_at",
        )
        .eq("published", true)
...
function NytLayout({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, height: "100%" }}>
      ...
      <Divider />
      <div
        style={{ display: "grid", gridTemplateColumns: `repeat(${cols}, 1fr)`, gap: 22, flex: 1 }}
      >
        {rest.map((it, i) => (
          <div
            key={it.postId}
            style={{
              borderLeft: i === 0 ? "none" : "1px solid #111111",
...
function LeadArticle({
  ...
  return (
    <article
      style={{ display: "flex", flexDirection: "column", gap: 10, flex: fullPage ? 1 : "initial" }}
    >
      <div
        style={{
          fontSize: 10,
...
function SmallArticle({ item }: { item: Selection & { post: Post } }) {
  ...
  return (
    <article style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {p.author && (
        <div
          style={{ fontSize: 9, letterSpacing: 2, textTransform: "uppercase", color: "#555555" }}
        >
          Por {p.author}
        </div>
      )}
...
export function AdminNewspaperPage() {
  return (
    <AdminShell>
      <NewspaperBuilder />
    </AdminShell>
